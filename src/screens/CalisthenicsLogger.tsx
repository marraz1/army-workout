...
export default function CalisthenicsLogger() {
  ...
  function handleSetDone(actualReps: number) {
    ...
    const exId =
      plan.source === "library" ? String(plan.libraryExerciseId) : (plan.customExerciseId ?? "");
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500">
                      {plan.sets}×{plan.repsOrSecs}
                      {ex.isHold ? "s" : ""} · {plan.restSec}s rest
                    </div>
                  </div>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-24 px-4 py-6">
        <div className="mb-4">
          <div className="text-xs font-bold uppercase text-purple-600">
            Exercise {todayPlans.findIndex((p) => p.id === current.plan.id) + 1} of{" "}
            {filteredQueue.map((q) => q.plan.id).filter((v, i, a) => a.indexOf(v) === i).length}
          </div>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-24 px-4 py-6 space-y-6">
        <div className="text-center">
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-800 dark:text-slate-100">
            {t("calisthenics.sessionComplete")}
          </h2>
          <p className="text-sm text-slate-500 mt-1">
            {t("calisthenics.setsCompleted", { completed, total })}
          </p>
        </div>
