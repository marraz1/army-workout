...
export default function WorkoutLogger() {
  ...
  const effective = useMemo(() => resolveEffectivePlan(group, plans, date), [group, plans, date]);
  ...
  const selected = useMemo(() => new Set(selectedIds ?? defaultIds), [selectedIds, defaultIds]);
  ...
  const summary = useMemo(() => computeSessionSummary(results, runLog), [results, runLog]);
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        on
                          ? "border-navy bg-navy dark:border-flag-yellow dark:bg-flag-yellow dark:text-navy"
                          : "border-slate-300",
                      )}
                    >
                      {on ? "✓" : ""}
        ...
        <Card>
          ...
          <div className="mt-4 flex gap-3">
            <Button
              ...
              disabled={!!runStr && parseMMSS(runStr) == null}
            >
              {t("common.next")} →
            </Button>
            <Button
      ...
      {phase === "saved" && (
        <Card>
          <div className="py-4 text-center">
            ...
            <div className="mt-2 text-lg font-bold text-navy dark:text-slate-100">
              {t("logger.saved")}
            </div>
            {newPBs.length > 0 && (
