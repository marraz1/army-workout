...
export default function CalisthenicsPlanner() {
  ...
  const exerciseIdRaw = editingPlan
    ? editingPlan.source === "library"
      ? String(editingPlan.libraryExerciseId)
      : (editingPlan.customExerciseId ?? "0")
  ...
  const [repsOrSecs, setRepsOrSecs] = useState(
    libEx?.defaultReps ?? custEx?.defaultRepsOrSecs ?? 10,
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-24">
      ...
      <div className="px-4 py-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-slate-500">
                {isTimed ? t("calisthenics.secondsLabel") : t("calisthenics.repsLabel")}
              </span>
            </div>
          </div>
        )}
        ...
        {/* Step 6: Rest */}
        {step === 6 && <RestPicker value={rest} onChange={setRest} />}
        ...
        {step === 7 && (
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800 space-y-3">
              ...
              <Row
                label={t("calisthenics.reviewSets")}
                value={`${sets} ${t("calisthenics.setsLabel")}`}
              />
              <Row
                label={isTimed ? t("calisthenics.holdToggle") : t("calisthenics.repsLabel")}
                value={`${repsOrSecs}${isTimed ? "s" : ""}`}
              />
        ...
        <div className="flex gap-3">
          ...
          {step < 7 ? (
            ...
            >
              {saving
                ? t("calisthenics.saving")
                : editingPlan
                  ? `✓ ${t("calisthenics.updatePlan")}`
                  : `💾 ${t("calisthenics.saveToSchedule")}`}
            </button>
          )}
        </div>
      </div>
