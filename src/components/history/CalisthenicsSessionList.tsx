...
export function CalisthenicsSessionList({ logs, muscleFilter }: CalisthenicsSessionListProps) {
  ...
  const grouped = useMemo(() => {
    const visible = muscleFilter ? logs.filter((l) => logMatchesMuscle(l, muscleFilter)) : logs;
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500">
                  {t("calisthenics.sessionSummary", { exercises: byEx.size, sets: completed })}
                </div>
              </div>
              <span
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {exLogs
                          ...
                          .map((l) => (
                            ...
                            >
                              {t("calisthenics.set", { n: l.setNumber })}: {l.actualReps}
                              {l.completed ? " ✓" : " ✗"}
                            </div>
                          ))}
                      </div>
